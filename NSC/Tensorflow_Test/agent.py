...
#입력으로 들어온 엡실론의 확률로 무작위로 행동을 결정, 그렇지 않은 경우 정책 신경망을 통해 행동 결정.
...
#정책 신경망이 결정한 행동의 확률이 높을수록 매수 또는 매도하는 단위를 크게 결정
...
#에이전트가 결정한 행동을 수행
    #action은 탐험 또는 정책 신경망을 통해 결정한 행동으로 매수와 매도를 의밓는 0 과 1
    #confidence는 정책 신경망을 통해 결정한 경우 결정한 행동에 대한 소프트맥스 확률 값
