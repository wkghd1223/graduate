...
- 결과 메모장 저장 -> 엑셀로 저장 
'''''''''''''''''''''''''''''''''''''''''''''''''''''''''
RESULT_PATH = 'C:/test/' #메모장 및 엑셀 파일 저장 경로
...
class crawling:
    ...
    def crawler(self, maxpage, query, s_date, e_date):
        ...
        f = open("C:/test/crawling_result.txt", 'w', encoding='utf-8')#open(파일 이름, 파일 열기모드 w=쓰기모드)

        while page < maxpage_t:#page가 maxpage_t보다 작을 동안

            print(page)

            url = "https://search.naver.com/search.naver?where=news&query=" + self.query + "&sort=0&ds=" + self.s_date + "&de=" + self.e_date + "&nso=so%3Ar%2Cp%3Afrom" + s_from + "to" + e_to + "%2Ca%3A&start=" + str(page)
